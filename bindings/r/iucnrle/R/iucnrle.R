...
#' alone do not produce a listing: at least one of sub-conditions (a) continuing
#' decline, (b) threatening processes, or (c) few threat-defined locations must
#' also hold.
#'
#' Sub-conditions you do not name count as *not assessed*, which is deliberately
#' different from *not met*. An ecosystem whose metrics say EN but whose
#' sub-conditions nobody has examined is reported as `"EN (LC-EN)"`, not a bare
#' `"EN"` that would overstate confidence.
#'
...
#' @param subconditions Named character vector, e.g. `c(a = "met")`. Values must
#'   be one of `"met"`, `"not_met"`, or `"not_assessed"`.
...
#' criterion_b(eoo_km2 = 15000, subconditions = c(a = "met"))$overall
#'
#' @export
criterion_b <- function(eoo_km2 = NULL,
                        aoo_cells = NULL,
                        subconditions = character(0),
                        eoo_bounds = NULL,
                        aoo_bounds = NULL) {
  ...
  if (length(subconditions) > 0 && is.null(names(subconditions))) {
    stop("`subconditions` must be a named vector, e.g. c(a = \"met\")", call. = FALSE)
  }

  json <- rle_criterion_b_json(
    eoo_km2 = as_scalar(eoo_km2),
    aoo_cells = as_scalar(aoo_cells),
    sub_names = as.character(names(subconditions)),
    sub_statuses = as.character(unname(subconditions)),
